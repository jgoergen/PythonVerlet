...
def distance(x, y):
    ...
    if x in distanceCache:
        if y not in distanceCache[x]:
            distanceCache[x].update({y: sqrt(x * x + y * y)})
    else:
        distanceCache[x] = {y: sqrt(x * x + y * y)}

    # can we flip x and y?
    if y in distanceCache:
        if x not in distanceCache[y]:
            distanceCache[y].update({x: sqrt(y * y + x * x)})
    else:
        distanceCache[y] = {x: sqrt(y * y + x * x)}
