...
class Line2D(object):
    ...
    def length(self):
        return sqrt(
            pow(self.ends.startVect2D.x - self.ends.endVect2D.x, 2) +
            pow(self.ends.startVect2D.y - self.ends.endVect2D.y, 2))
    ...
    def direction(self):
        # radians
        return pi + atan2(self.ends.endVect2D.x + self.ends.startVect2D.x, self.ends.endVect2D.y - self.ends.startVect2D.y)
